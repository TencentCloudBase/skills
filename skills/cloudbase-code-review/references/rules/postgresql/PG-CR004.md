...
- 如果上传逻辑中出现 `storage.upload({ cloudPath, filePath })`：记录 **warning**。PG 场景推荐 Storage v3 的 `upload("bucket/path", file)` 参数形态。
...
2. 上传实现是否优先使用 `app.storage.from().upload("bucket/path", file)`，并把 bucket 放在 path 第一段？
...
const { data, error } = await storage.upload(`${bucketName}/${path}`, file)
...
CloudBase PG Web 场景中，文件/图片上传更适合走 Web Storage v3 API：`app.storage.from().upload("bucket/path", file)`，其中 bucket 必须是 path 的第一段。agent 容易从旧示例、顶层 storage API 或类型片段误推成 `app.storage()`、`app.uploadFile(...)` 或 `storage.upload({ cloudPath, filePath })`。其中 `app.storage()` 是确定性运行时错误；旧式 API 则不符合当前 PG 场景的推荐路径。
