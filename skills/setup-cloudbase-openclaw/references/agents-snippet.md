...
6. **Web hosting: deploy to a subdirectory (mandatory)** — When deploying a web app to static hosting, **you must** deploy under a **subdirectory** (e.g. `/my-app/`), not the root, unless the user explicitly asks otherwise. After deployment, **always return the full access URL with trailing slash**.
